...
#include "../../Include/All/Scenes/LoseScene.hpp"
#include "../../Include/All/interfaces/ISprite.hpp"
#include <iostream>
#include <algorithm>

const std::string LOSE_BG_PATH = "Assets/sprites/parallax.jpg";
static size_t bg_texture_id;
static size_t font_id;

void LoseScene::init(GameEngine& engine)
{
    std::cout << "LoseScene: Initializing..." << std::endl;

    bg_texture_id = engine.getResourceManager().load_texture(LOSE_BG_PATH);
    ...
    float current_score = 1200;
    float high_score = 5500;
    ...
    Entity lose_label = _registry.spawn_entity();
    lose_label.set_name("LoseLabel");
    lose_label.add_component<Component::Core::Position>(
        Component::Core::Position(CENTER_X - 230, WINDOW_HEIGHT / 2.5f));
    lose_label.add_component<Component::Graphics::UIText>(
        Component::Graphics::UIText("YOU LOSE :", font_id, 60, Component::Core::Color(255, 80, 80)));

    Entity lose_value = _registry.spawn_entity();
    lose_value.set_name("LoseValue");
    lose_value.add_component<Component::Core::Position>(
        Component::Core::Position(CENTER_X + 290, WINDOW_HEIGHT / 2.5f));
    lose_value.add_component<Component::Graphics::UIText>(
        Component::Graphics::UIText(std::to_string(static_cast<int>(current_score)), font_id, 60, Component::Core::Color(255, 215, 0)));
    ...
    std::cout << "LoseScene initialized for window " << WINDOW_WIDTH << "x" << WINDOW_HEIGHT << std::endl;
}

void LoseScene::update(GameEngine& engine, float delta_time)
{
...
void LoseScene::render(GameEngine& engine)
{
