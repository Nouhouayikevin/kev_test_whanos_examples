...
// NOTE: Pas de ResourceManager sur le serveur (pas de rendu graphique)
// Les textures et sons ne sont pas chargés côté serveur
...
Snapshot OnlineGameScene_server::init_entity_player(GameEngine& gameSceneEngine, uint32_t player_id, uint32_t tick) {
    ...
    gameSceneEngine.getRegistry().add_component(player, Component::Core::Hitbox(32, 32));
    
    // Graphics components - Server doesn't load textures but must send sprite data for clients
    // Sprite(texture_id, frame_width, frame_height, z_index)
    gameSceneEngine.getRegistry().add_component(player, Component::Graphics::Sprite(1, 33, 21, 10));
    gameSceneEngine.getRegistry().add_component(player, Component::Graphics::Animation(0, 5, 1.0f, true));
    
    // Gameplay components - FIX: Use player_id instead of 0
    ...
    health_bar.set_name("Player" + std::to_string(player_id) + "_HealthBar");
    gameSceneEngine.getRegistry().add_component(health_bar, Component::UI::HealthBar(player, 0, -20.0f));
    gameSceneEngine.getRegistry().add_component(health_bar, Component::Core::Position(0, 0)); 
    gameSceneEngine.getRegistry().add_component(health_bar, Component::Core::Hitbox(32, 5));  
    gameSceneEngine.getRegistry().add_component(health_bar, Component::Graphics::RectangleShape(Component::Core::Color(0, 255, 0, 220)));

    // Create snapshot using SnapshotHelper
    std::vector<Entity> entities = {player, health_bar};
    Snapshot snapshot = SnapshotHelper::createSnapshot(entities, gameSceneEngine.getRegistry(), tick);

    return snapshot;
}

void OnlineGameScene_server::init(GameEngine& gameSceneEngine) {
    ...
    gameSceneEngine.getRegistry().add_system([this](GameEngine& gameEng, float dt) {
        ...
        auto const& player_tags = gameEng.getRegistry().get_components<Component::Gameplay::PlayerTag>();
    
        const uint8_t UP_BIT    = 1 << 0;
        const uint8_t DOWN_BIT  = 1 << 1;
        const uint8_t LEFT_BIT  = 1 << 2;
        const uint8_t RIGHT_BIT = 1 << 3;
        const uint8_t SHOOT_BIT = 1 << 4;
  
        // DEBUG: Afficher le nombre d'inputs dans la queue
        if (!_inputQueue.empty()) {
            std::cout << "[INPUT SYSTEM] Processing " << _inputQueue.size() << " inputs" << std::endl;
        }
  
        while (!_inputQueue.empty()) {
            auto input_opt = _inputQueue.pop();
            if (!input_opt) break;

            Input input = *input_opt;
            uint32_t player_id = input.playerId;
            uint8_t input_bits = input.getInput();

            for (size_t i = 0; i < velocities.size() && i < controllables.size() && i < player_tags.size(); i++) {
                auto const& dead_tags = gameEng.getRegistry().get_components<Component::Gameplay::Dead>();
                if (i < dead_tags.size() && dead_tags[i]) continue;
                if (!velocities[i] || !controllables[i] || !player_tags[i]) continue;
                if (player_tags[i]->player_id != player_id) continue;

                velocities[i]->dx = 0;
                velocities[i]->dy = 0;
                float player_speed = 200.0f;

                if (input_bits & UP_BIT) {
                    velocities[i]->dy = -player_speed;
                }
                if (input_bits & DOWN_BIT) {
                    velocities[i]->dy = player_speed;
                }
                if (input_bits & LEFT_BIT) {
                    velocities[i]->dx = -player_speed;
                }
                if (input_bits & RIGHT_BIT) {
                    velocities[i]->dx = player_speed;
                }

                if (input_bits & SHOOT_BIT) {
                    ...
                    if (i < positions.size() && positions[i]) {
                        ...
                        gameEng.getRegistry().add_component(missile, Component::Gameplay::MissileTag(player_id));
    ...
    // NOTE: Système audio désactivé sur le serveur (pas de rendu sonore)
    // gameSceneEngine.getRegistry().add_system([](GameEngine& gameEng ,float dt) {
...
void OnlineGameScene_server::update(GameEngine& gameSceneEngine, float delta_time) {
    ...
    if (_enemy_spawn_timer <= 0.0f) {
        ...
        // Sprite(texture_id, frame_width, frame_height, z_index)
        gameSceneEngine.getRegistry().add_component(enemy, Component::Graphics::Sprite(2, 32, 33, 5));
        ...
        gameSceneEngine.getRegistry().add_component(enemy, Component::Gameplay::Score(10)); // Score value for killing this enemy
...
void OnlineGameScene_server::render(GameEngine& gameEngine) {
