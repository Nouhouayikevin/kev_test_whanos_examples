...
#include "../../Include/All/Scenes/ScoreScene.hpp"
#include "../../Include/All/interfaces/ISprite.hpp"
#include <iostream>
#include <algorithm>

const std::string BG_LAYER_PATH = "Assets/sprites/parallax.jpg";
static size_t bg_texture_id;
static size_t font_id;

void ScoreScene::init(GameEngine& engine)
{
    std::cout << "ScoreScene: Initializing..." << std::endl;

    bg_texture_id = engine.getResourceManager().load_texture(BG_LAYER_PATH);
    ...
    Entity score_label = _registry.spawn_entity();
    score_label.set_name("ScoreLabel");
    score_label.add_component<Component::Core::Position>(
        Component::Core::Position(CENTER_X - 180, WINDOW_HEIGHT / 3.0f));
    score_label.add_component<Component::Graphics::UIText>(
        Component::Graphics::UIText("SCORE:", font_id, 60, Component::Core::Color(255, 255, 255)));

    Entity score_value_text = _registry.spawn_entity();
    score_value_text.set_name("ScoreValue");
    score_value_text.add_component<Component::Core::Position>(
        Component::Core::Position(CENTER_X + 250, WINDOW_HEIGHT / 3.0f));
    score_value_text.add_component<Component::Graphics::UIText>(
        Component::Graphics::UIText(std::to_string(static_cast<int>(score_value)), font_id, 60, Component::Core::Color(255, 255, 0)));

    Entity next_level = _registry.spawn_entity();
    next_level.set_name("NextLevelButton");
    next_level.add_component<Component::Core::Position>(
        Component::Core::Position(CENTER_X - 200, WINDOW_HEIGHT - 140));
    next_level.add_component<Component::Graphics::UIText>(
        Component::Graphics::UIText("NEXT LEVEL", font_id, 50, Component::Core::Color(0, 255, 0)));

    std::cout << "ScoreScene initialized for window " << WINDOW_WIDTH << "x" << WINDOW_HEIGHT << std::endl;
}

void ScoreScene::update(GameEngine& engine, float delta_time)
{
...
void ScoreScene::render(GameEngine& engine)
{
